...
#include <map>
typedef std::map<HWND, CD3DApp*>	mpWINOBJ;
typedef mpWINOBJ::iterator			itWINOBJ;

static mpWINOBJ mp_obj_win;

static CD3DApp* FindObjectWindow(HWND hWnd)
{
	itWINOBJ it = mp_obj_win.find(hWnd);
...
static int InsertObjectWindow(HWND hWnd, CD3DApp* obj)
{
	CD3DApp* old_obj = FindObjectWindow(hWnd);
	if(old_obj)
		return -1;
...
LRESULT WINAPI CD3DApp::WndProc( HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam )
{
	switch(uMsg)
	{
		case WM_CREATE:
		{
			...
			CD3DApp* win_obj = NULL;
			if(st && st->lpCreateParams)
			{
				win_obj = (CD3DApp*)st->lpCreateParams;
		...
		case WM_DESTROY:
		{
			CD3DApp* win_app = FindObjectWindow(hWnd);
			if(win_app)
			{
				DeleteObjectWindow(hWnd);
				return win_app->MsgProc(hWnd, uMsg, wParam, lParam);
			}
